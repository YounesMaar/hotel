...
@endphp
<div
    class="@if ($has_border && !$celled) border border-gray-200/70 dark:border-dark-700/60 @endif border-collapse max-w-full">
    <div class="w-full">
        @if ($searchable)
            <div class="bw-table-filter-bar">
                <x-bladewind::input name="bw-search-{{ $name }}" placeholder="{{ $search_placeholder }}"
                    onkeyup="filterTable(this.value, 'table.{{ $name }}')" add_clearing="false"
                    class="!mb-0 focus:!border-slate-300 !pl-9 !py-3" clearable="true" prefix_is_icon="true"
                    prefix="magnifying-glass" />
            </div>
        @endif

        <table
            class="bw-table w-full {{ $name }} @if ($has_shadow) drop-shadow shadow shadow-gray-200/70 dark:shadow-md dark:shadow-dark-950/20 @endif
                                        ...
                                        @foreach ($icons_array as $icon)
                                            @if (isset($icon['icon']))
                                                ...
                                                <x-bladewind::button.circle size="tiny" icon="{{ $icon['icon'] }}"
                                                    color="{{ $icon['color'] ?? '' }}" {{-- title="{{$icon['tip']??''}}" --}}
                                                    onclick="{!! build_click($icon['click'], $row) ?? 'void(0)' !!}" type="{!! isset($icon['color']) ? 'primary' : 'secondary' !!}" />
                                                @if (!empty($icon['tip']))
                                                    </a>
                                                @endif
                                            @endif
                        ...
                        <tr>
                            <td colspan="{{ count($table_headings) }}" class="text-center">
                                @if ($message_as_empty_state)
                                    <x-bladewind::empty-state :message="$no_data_message" :button_label="$button_label" :onclick="$onclick"
                                        :image="$image" :show_image="$show_image" :heading="$heading" />
                                @else
                                    {{ $no_data_message }}
                                @endif
                                <script>
