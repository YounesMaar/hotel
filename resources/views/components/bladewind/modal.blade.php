@props([
    ...
    'okButtonLabel' => config('bladewind.modal.ok_button_label', 'confirm'),
...
@php
    ...
    if ($okButtonLabel !== $ok_button_label) {
        $ok_button_label = $okButtonLabel;
    }
    if ($okButtonAction !== $ok_button_action) {
        $ok_button_action = $okButtonAction;
    }
    if ($cancelButtonLabel !== $cancel_button_label) {
        $cancel_button_label = $cancelButtonLabel;
    }
    if ($cancelButtonAction !== $cancel_button_action) {
    ...
    if ($centerActionButtons) {
        $center_action_buttons = $centerActionButtons;
    }
    if ($stretchActionButtons) {
        $stretch_action_buttons = $stretchActionButtons;
    }
    if ($blurBackdrop) {
    ...
    if (!in_array($align_buttons, ['right', 'center', 'left'])) {
        $align_buttons = 'right';
    }
    //-------------------------------------------------------------------
    ...
    if ($ok_button_action !== 'close') {
        $okAction = $ok_button_action . ($close_after_action ? ';' . $okAction : '');
    }
...
<div data-name="{{ $name }}" data-backdrop-can-close="{{ $backdrop_can_close }}"
    ...
    <div class="sm:{{ $sizes[$size] }} lg:{{ $sizes[$size] }} p-4 m-auto bw-{{ $name }} animate__faster">
        <div
            class="bg-white relative dark:bg-dark-700/90 dark:border dark:border-dark-500/10 rounded-lg drop-shadow-2xl">
            ...
            <div class="{{ !empty($type) || !empty($icon) ? 'flex' : 'flex-initial' }} p-5 ">
                ...
                <div class="modal-body grow px-2 {{ $body_css }}">
                    <h1
                        class="text-lg font-semibold leading-5 text-gray-900 dark:text-dark-400 tracking-wide modal-title text-left pb-0.5">
                        {{ $title }}</h1>
            ...
            @if ($show_action_buttons)
                <div
                    class="modal-footer @if ($stretch_action_buttons) flex flex-col-reverse @endif
                ...
                bg-gray-100 dark:bg-dark-800/50 border-t border-t-gray-200/60 dark:border-t-dark-600/50 py-3 px-6 rounded-br-lg rounded-bl-lg {{ $footer_css }}">
                    <x-bladewind::button type="secondary" size="{{ $button_size }}" onclick="{!! $cancelAction !!}"
                        class="cancel {{ $stretch_action_buttons ? 'block w-full mb-3' : '' }} {{ $cancelCss }}">{{ $cancel_button_label }}</x-bladewind::button>

                    <x-bladewind::button size="{{ $button_size }}" onclick="{!! $okAction !!}"
